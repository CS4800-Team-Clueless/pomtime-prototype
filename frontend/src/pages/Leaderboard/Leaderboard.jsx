...
const CHARACTER_RARITY = {
  King: 5, Angel: 5, Dragon: 5,
  Snow: 4, Prince: 4, Moon: 4, Autumn: 4,
  White: 3, Brown: 3, Orange: 3, Black: 3, Cream: 3, Gray: 3, Tan: 3, Beige: 3
};
...
export default function Leaderboard() {
  ...
  const loadFriendsAndLeaderboard = async () => {
    ...
    try {
      ...
      const leaderboardResponse = await fetchWithAuth(`${API_URL}/api/friends/leaderboard`);
  ...
  const handleRemoveFriend = async (email) => {
    ...
    try {
      const response = await fetchWithAuth(`${API_URL}/api/friends/${encodeURIComponent(email)}`, {
        method: 'DELETE'
      });
  ...
  const handleViewProfile = async (email) => {
    try {
      const response = await fetchWithAuth(`${API_URL}/api/user/public-profile`, {
        method: 'POST',
        body: JSON.stringify({ email })
      });
  ...
  return (
    <div className="leaderboard-page">
        ...
        {loading ? (
          ...
          <div className="leaderboard-list">
            <div className="list-header">
              <h2 className="list-title">Your Friends ({leaderboard.length})</h2>
            </div>
            {leaderboard.map((player, index) => (
              ...
              >
                ...
                >
                  <img
                    src={player.picture || `https://ui-avatars.com/api/?name=${encodeURIComponent(player.name)}&size=64&background=6366f1&color=fff`}
                    alt={player.name}
      ...
      {/* Profile Modal */}
      <Modal show={showProfileModal} onHide={closeProfileModal} centered size="lg">
          ...
          {selectedProfile && (
            <div className="profile-modal-content">
              <div className="profile-header">
                <img
                  src={selectedProfile.picture || `https://ui-avatars.com/api/?name=${encodeURIComponent(selectedProfile.name)}&size=128&background=6366f1&color=fff`}
                  alt={selectedProfile.name}
                  className="profile-avatar"
                />
                <div className="profile-details">
                  <h3>{selectedProfile.name}</h3>
                  <p className="profile-email">{selectedProfile.email_display}</p>
                  <div className="profile-level">
                    <span className="level-badge-large">Level {selectedProfile.level}</span>
                    <span className="xp-text">{selectedProfile.experience} XP</span>
                  </div>
                </div>
              </div>
              ...
              <div className="profile-stats-grid">
                <div className="profile-stat-card">
                  ...
                  <div className="stat-value">{selectedProfile.stats.total_pomeranians}</div>
                  <div className="stat-label">Total Pomeranians</div>
                </div>
                <div className="profile-stat-card">
                  <div className="stat-icon">✨</div>
                  <div className="stat-value">{selectedProfile.stats.unique_pomeranians}</div>
                  <div className="stat-label">Unique Collected</div>
                </div>
              </div>

              {selectedProfile.displayed_characters && selectedProfile.displayed_characters.length > 0 && (
                <div className="displayed-section">
                  ...
                  <div className="displayed-grid">
                    {selectedProfile.displayed_characters.map((charName) => {
                      ...
                      return (
                        <div key={charName} className={`displayed-char rarity-${rarity}`}>
                          <img
                            src={GACHA_ART[charName] || DEFAULT_ART}
                            alt={charName}
                            className="displayed-char-img"
                          />
                          <div className="displayed-char-name">{charName}</div>
                          <div className="displayed-char-stars">{'★'.repeat(rarity)}</div>
                        </div>
                      );
                    })}
                  </div>
